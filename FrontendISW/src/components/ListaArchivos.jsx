...
const api = axios.create({
  baseURL: 'http://146.83.198.35:1725'
});
...
useEffect(() => {
  ...
  const eliminarArchivo = async (archivoId) => {
    try {
      ...
      if (confirmed) {
        const response = await axios.delete(`http://146.83.198.35:1725/documentos/${archivoId}`);
        ...
        <tbody>
          {archivos.map(archivo => (
            <tr key={archivo._id}>
              ...
              <td className="table-data-cell">
              <button onClick={() => window.open(`http://146.83.198.35:1725/documentos/descargar/${archivo._id}`, '_blank')} type="button" class="btn btn-success custom-btn btn-sm" style={{width: '70px', textAlign:'center', padding:'4px 0'}}>
              Descargar
              </button>
              <button onClick={() => window.open(`http://146.83.198.35:1725/documentos/ver/${archivo._id}`, '_blank')} type="button" class="btn btn-secondary custom-btn btn-sm">
              Ver
              </button>
